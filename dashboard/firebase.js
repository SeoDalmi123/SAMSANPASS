        ...
        onAuthStateChanged(auth, (user) => {
            if (user) {
                ...
                if (!user.emailVerified) {
                    ...
                    unverified.innerHTML = "Actions blocked. Please check your email to verify your account. Refresh window after verifying";
                    unverified.style.position = "fixed";
                    unverified.style.display = "block"
                    unverified.style.top = "0";
                    unverified.style.left = "0";
                    unverified.style.textAlign = "center";
                    unverified.style.zIndex = "99999";
                    unverified.style.height = "auto";
                    unverified.style.width = "100%";
                    unverified.style.backgroundColor = "orange";
                    unverified.style.fontSize = ".8rem"
                    mainBody.style.pointerEvents = "none";
                    body.appendChild(unverified);
        ...
        async function GetAllDataRealtime(){
            ...
            const unsubscribe = onSnapshot(q,(querySnapshot) => {
                ...
                /* inputData.forEach((doc)=>{
                    var savedDate = doc.date.toDate('en-US');
                    var endDate = savedDate.setMonth(savedDate.getMonth() + 3);
                    var current = new Date().toDateString();
                    console.log(savedDate.toDateString());
                    console.log(endDate);
                    console.log(current);
                }); */
                 /* const docRef = doc(db, "users", user.uid,"websites",expireWebsite);
                    ...
                    tableBody.forEach((row) => {
                        ...
                        const storedEndDate = localStorage.getItem(`endDate-${expireWebsite}`);
                        if (storedEndDate) {
                            endDate = new Date(storedEndDate);
                        } else {
                            endDate.setMonth(endDate.getMonth() + 3);
                            localStorage.setItem(`endDate-${expireWebsite}`, endDate.toString());
                        }
                        ...
                        var countDownInterval = setInterval(function() {
                            ...
                            if (remainingTime >= 0) {
                                ...
                                if (remainingTime <= 604800000) {
                                    ...
                                    popup.innerHTML = "Change password (URGENT): " + " " + dateString + " " + year + " " + "(" + remainingTimeString + ")";
                                }
                            } else {
                        ...
                        // Add listener for when the password value changes
                        expirePassword.addEventListener("change", function() {
                            endDate = new Date();
                            endDate.setMonth(endDate.getMonth() + 3);
                            localStorage.setItem(`endDate-${expireWebsite}`, endDate.toString());
                        });
